...
export default function Footer() {
  ...
  return (
    <footer
      ...
      // Explicit solid bg prevents transparency bleed from sections above
      style={{ backgroundColor: '#000000', borderTop: '1px solid rgba(223,242,69,0.08)' }}
    >
      {/* Top glow line */}
      <div
        className="absolute top-0 left-0 right-0 h-px pointer-events-none"
        style={{ background: `linear-gradient(90deg, transparent 0%, ${C}30 30%, ${C}70 50%, ${C}30 70%, transparent 100%)` }}
      />

      {/* Ambient glow — lightweight radial, no blur filter */}
      <div
        className="absolute pointer-events-none"
        style={{ top: '-100px', left: '50%', transform: 'translateX(-50%)', width: '500px', height: '260px', background: `radial-gradient(ellipse, ${C}05 0%, transparent 70%)` }}
      />
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ width: '40px', height: '40px', border: `1px solid ${C}40`, borderRadius: '10px', background: `${C}08` }}
              >
                <span style={{ color: C, fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '14px' }}>S</span>
                <div className="absolute inset-0 rounded-[10px] pointer-events-none" style={{ boxShadow: `0 0 16px ${C}15` }} />
              </div>
              <div>
            ...
            >
              <div
                ...
                style={{ width: '6px', height: '6px', background: P, boxShadow: `0 0 6px ${P}`, animation: 'pulse-dot 2s ease-in-out infinite' }}
              />
            ...
            {navLinks.map((link, i) => (
              ...
              >
                <div
                  ...
                  style={{ width: '12px', background: `${C}40` }}
                />
                <span
                  style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', fontFamily: 'DM Sans, sans-serif', fontWeight: 500, letterSpacing: '0.05em', transition: 'color 0.3s ease' }}
              ...
              <div className="flex flex-wrap gap-2">
                {techStack.map((tech, i) => (
                  <motion.span
                    ...
                    style={{ padding: '3px 10px', fontSize: '10px', fontWeight: 600, color: 'rgba(255,255,255,0.45)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '4px', background: 'rgba(255,255,255,0.025)', fontFamily: 'monospace' }}
                  >
                    {tech}
                  </motion.span>
                ))}
              ...
              <div className="flex gap-3">
                {socialLinks.map((s, i) => (
                  <motion.a
                    ...
                    style={{ width: '38px', height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '10px', background: 'rgba(255,255,255,0.025)', color: 'rgba(255,255,255,0.4)', textDecoration: 'none', transition: 'all 0.3s ease' }}
                    onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = `${C}50`; el.style.color = C; el.style.background = `${C}0d`; el.style.boxShadow = `0 0 12px ${C}18`; el.style.transform = 'translateY(-2px)' }}
                    onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = 'rgba(255,255,255,0.07)'; el.style.color = 'rgba(255,255,255,0.4)'; el.style.background = 'rgba(255,255,255,0.025)'; el.style.boxShadow = 'none'; el.style.transform = 'translateY(0)' }}
                  >
                    {s.icon}
                  </motion.a>
                ))}
