...
// Reduced particle count for shared bg canvas
...
// Robot — pointer-events fully removed; purely decorative
function RobotFace() {
  ...
  useFrame(({ clock }) => {
    ...
    if (headRef.current) {
      // Auto slow sway — no pointer tracking needed (passive animation only)
    ...
    if (actionTimer.current > nextActionInterval.current) {
      robotState.current = robotState.current === 'idle'
        ? (THREE.MathUtils.randInt(0, 1) === 0 ? 'wink' : 'smile')
        : 'idle'
      actionTimer.current = 0
      nextActionInterval.current = robotState.current === 'idle'
  ...
  // Reduced grill bar count: 28 bars (was 28, kept same but simplified geometry reuse)
  ...
  return (
    <group ref={headRef}>
      <pointLight position={[-3, 3, 4]}     color={C} intensity={2.5} distance={10} />
      <pointLight position={[3, 2, -2]}     color={M} intensity={1.6} distance={9} />
      <pointLight position={[0, -2.5, 3.5]} color="#fff" intensity={0.4} distance={6} />
      ...
      <pointLight position={[-0.2, 0.1, 0.72]} color={C} intensity={2} distance={1.5} />
      <pointLight position={[ 0.2, 0.1, 0.72]} color={C} intensity={2} distance={1.5} />
      <mesh position={[0, -0.31, 0.37]} material={mMid}><boxGeometry args={[0.72, 0.2, 0.36]} /></mesh>
      <group>
        {grilleBars.map((geo, i) => (
          <mesh key={i} geometry={geo} material={mAcc} />
        ))}
      </group>
      <mesh position={[ 0.57, 0.04, 0.04]} material={mAcc}><boxGeometry args={[0.046, 0.38, 0.58]} /></mesh>
      <mesh position={[-0.57, 0.04, 0.04]} material={mAcc}><boxGeometry args={[0.046, 0.38, 0.58]} /></mesh>
      <mesh position={[ 0.612, 0.08, 0.04]} rotation={[0, Math.PI / 2, 0]} material={mLight}><cylinderGeometry args={[0.06, 0.06, 0.03, 10]} /></mesh>
      <mesh position={[-0.612, 0.08, 0.04]} rotation={[0, Math.PI / 2, 0]} material={mLight}><cylinderGeometry args={[0.06, 0.06, 0.03, 10]} /></mesh>
      <mesh position={[0, -0.46, 0.3]}  material={mDark}><boxGeometry args={[0.56, 0.09, 0.26]} /></mesh>
      <mesh position={[0, -0.56, 0]}    material={mMid}><cylinderGeometry args={[0.27, 0.29, 0.11, 12]} /></mesh>
      <mesh position={[0, -0.7, 0]}     material={mDark}><cylinderGeometry args={[0.22, 0.24, 0.2, 12]} /></mesh>
      <mesh position={[0, 0.59, 0]}     material={mMid}><cylinderGeometry args={[0.022, 0.03, 0.33, 6]} /></mesh>
    ...
    >
      ...
      >
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '14px', marginBottom: '0.45rem', flexWrap: 'wrap' }}>
            <motion.h3
              ...
              style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, margin: 0, fontSize: 'clamp(1.05rem, 2.4vw, 1.6rem)', letterSpacing: '-0.03em', lineHeight: 1 }}
            >
              {project.title}
            </motion.h3>
            <span style={{ fontSize: '8.5px', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.22)', border: '1px solid rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '3px', whiteSpace: 'nowrap' }}>
              {project.category}
            </span>
          </div>
          <p style={{ color: 'rgba(255,255,255,0.36)', fontSize: '12.5px', lineHeight: 1.65, fontWeight: 300, margin: '0 0 0.8rem', maxWidth: '520px' }}>
            {project.shortDesc}
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
            {project.tech.map(t => (
              <motion.span
                ...
                transition={{ duration: 0.3 }}
                style={{ padding: '3px 9px', fontSize: '10px', fontWeight: 500, border: '1px solid', borderRadius: '3px' }}
              >
                {t}
              </motion.span>
            ))}
        ...
        <motion.div
          ...
          style={{ width: '40px', height: '40px', borderRadius: '10px', border: '1px solid', display: 'flex', alignItems: 'center', justifyContent: 'center', color: project.color === W ? '#dff245' : project.color, flexShrink: 0 }}
        >
...
export default function ClassyProjectsSection() {
  ...
  const handleOpenProject = useCallback((project: typeof projects[0]) => { setActiveProject(project) }, [])
      ...
      {mounted && (
        <style dangerouslySetInnerHTML={{
          __html: `
            ...
            .projects-robot-wrapper { flex-shrink:0; width:clamp(130px,16vw,200px); height:clamp(150px,19vw,240px); position:relative; }
            @media(max-width:700px){
              ...
              .projects-robot-wrapper { width:160px; height:190px; order:-1; margin:0 auto; }
            }
          `,
        }} />
      )}

      {/* Particles bg — pointer/touch events fully disabled */}
      {mounted && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none', touchAction: 'none', opacity: 0.9 }}>
          <Canvas
            dpr={[1, 1.5]}
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '84rem', margin: '0 auto' }}>
        ...
        <div className="projects-header-row">
          ...
          >
            ...
            <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 'clamp(3.2rem, 8vw, 7rem)', letterSpacing: '-0.04em', lineHeight: 0.88, marginBottom: '1.8rem', color: W }}>
              Selected<br />
              <span style={{
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
                backgroundImage: `linear-gradient(90deg, ${W}, ${C}, ${P}, ${C}, ${W})`,
                backgroundSize: '200% auto', animation: 'tubelight-name 4s linear infinite', display: 'inline-block',
              }}>
                Production.
              </span>
            </h2>

            <p style={{ color: 'rgba(255,255,255,0.52)', fontSize: '1.05rem', fontWeight: 300, lineHeight: 1.75, maxWidth: '440px', fontFamily: 'Outfit, sans-serif' }}>
            ...
            <div style={{ display: 'flex', gap: '2.5rem', marginTop: '2.5rem' }}>
              {[['04', 'Projects'], ['3+', 'Years'], ['100%', 'Deployed']].map(([val, label]) => (
                <div key={label}>
                  <div style={{
                    fontFamily: 'Outfit, sans-serif', fontSize: '1.5rem', fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1,
                    background: `linear-gradient(90deg, ${C}, ${P}, ${C})`, backgroundSize: '200% auto',
                    WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
          ...
          {/* Robot — FULLY PASSIVE: pointer-events none, no user interaction */}
          <motion.div
            style={{ y: robotY }}
          ...
          >
            {mounted && (
              <div style={{ width: '100%', height: '100%', pointerEvents: 'none', touchAction: 'none' }}>
                <Canvas
                  dpr={[1, 1.5]}
                  camera={{ position: [0, 0, 3.1], fov: 42 }}
                  gl={{ antialias: true, alpha: true, powerPreference: 'high-performance' }}
                  style={{ width: '100%', height: '100%', background: 'transparent', pointerEvents: 'none', touchAction: 'none' }}
                >
                  <RobotFace />
                  <EffectComposer enableNormalPass={false} multisampling={0}>
                    <Bloom luminanceThreshold={0.1} mipmapBlur intensity={0.9} resolutionScale={0.5} />
                  </EffectComposer>
                </Canvas>
              </div>
            )}
        ...
        <div style={{ position: 'relative', zIndex: 10 }}>
          ...
          {projects.map((project, i) => (
            <ProjectCard key={project.id} project={project} index={i} onOpen={() => handleOpenProject(project)} />
          ))}
          <div style={{ width: '100%', height: '1px', background: 'rgba(255,255,255,0.06)' }} />
        </div>
      </div>
        ...
        {activeProject && (
          ...
          >
            <motion.div
              ...
              style={{
                background: '#0a0a0a', border: `1px solid ${activeProject.color === W ? '#dff245' : activeProject.color}30`,
                borderRadius: '16px', padding: '2.5rem', maxWidth: '560px', width: '100%',
                boxShadow: `0 24px 64px rgba(0,0,0,0.8)`,
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                <div>
                  <span style={{ fontSize: '10px', fontFamily: 'monospace', letterSpacing: '0.25em', color: activeProject.color === W ? '#dff245' : activeProject.color }}>{activeProject.num}</span>
                  <h3 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: '1.6rem', color: W, margin: '4px 0 6px', letterSpacing: '-0.03em' }}>{activeProject.title}</h3>
                  <span style={{ fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', border: '1px solid rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '3px' }}>{activeProject.category}</span>
                </div>
                <button onClick={() => setActiveProject(null)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem', lineHeight: 1.75, marginBottom: '1.5rem', fontFamily: 'Outfit, sans-serif', fontWeight: 300 }}>{activeProject.longDesc}</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {activeProject.tech.map(t => (
                  <span key={t} style={{ padding: '4px 12px', fontSize: '11px', fontWeight: 600, color: activeProject.color === W ? '#dff245' : activeProject.color, border: `1px solid ${activeProject.color === W ? '#dff245' : activeProject.color}35`, background: `${activeProject.color === W ? '#dff245' : activeProject.color}0a`, borderRadius: '4px' }}>{t}</span>
                ))}
              </div>
            </motion.div>
          </motion.div>
