...
function HologramContactNode() {
  ...
  const sphereGeo   = useMemo(() => new THREE.SphereGeometry(0.08, 10, 10), [])
...
export default function Contact() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. WhatsApp ka link taiyar karna
    const whatsappNumber = '923115341514' // Aapka number
    const whatsappText = `*New Lead from Portfolio*%0A*Name:* ${name}%0A*Message:* ${message}`
    const waFullUrl = `https://wa.me/${whatsappNumber}?text=${whatsappText}`
    
    try {
      // 3. EmailJS ko background mein trigger karna (Don't wait for it to finish before opening WA)
      ...
      // 4. Foran WhatsApp open karna (User ko wait nahi karwana)
      window.open(waFullUrl, '_blank')

      // 5. Ab email ka wait karo ke wo bhi confirm ho jaye
      await emailPromise
      ...
      setSending(false)
      alert('Email bhenjne mein masla hua, lekin WhatsApp open ho gaya hoga.')
    }
  }
  ...
  return (
    <section
      ...
      style={{ backgroundColor: '#000000', paddingTop: '8rem', paddingBottom: '10rem' }}
    >
      ...
      {/* BG Particles canvas — NO pointer/touch events */}
      <div
      ...
      {/* Subtle ambient radial glows — pure CSS, no GPU overhead */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <div style={{ position: 'absolute', top: '30%', left: '15%', width: 460, height: 460, borderRadius: '50%', background: `radial-gradient(circle,${C}05 0%,transparent 70%)`, transform: 'translate(-50%,-50%)' }} />
        <div style={{ position: 'absolute', bottom: '20%', right: '10%', width: 360, height: 360, borderRadius: '50%', background: `radial-gradient(circle,${M}05 0%,transparent 70%)`, transform: 'translate(50%,50%)' }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-20 lg:gap-32 justify-center">
          ...
          >
            <AnimatePresence mode="wait">
              {!sent ? (
                ...
                >
                  ...
                  <div className="relative group pt-4">
                    <input
                      ...
                      className="peer w-full text-white text-[15px] px-6 py-4 rounded-2xl outline-none transition-all duration-300"
                      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}
                      onFocus={e => { e.currentTarget.style.borderColor = C; e.currentTarget.style.background = `${C}0a`; e.currentTarget.style.boxShadow = `0 8px 20px -8px ${C}25` }}
                      onBlur={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.boxShadow = 'none' }}
                    />
                  ...
                  <div className="relative group pt-4">
                    <textarea
                      ...
                      className="peer w-full text-white text-[15px] px-6 py-4 rounded-2xl outline-none transition-all duration-300 resize-none"
                      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}
                      onFocus={e => { e.currentTarget.style.borderColor = C; e.currentTarget.style.background = `${C}0a`; e.currentTarget.style.boxShadow = `0 8px 20px -8px ${C}25` }}
                      onBlur={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.boxShadow = 'none' }}
                    />
                  ...
                  <button
                    type="submit"
                    disabled={sending}
                    className="w-full py-4 mt-4 rounded-2xl relative overflow-hidden transition-all duration-300 group"
                    style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', fontSize: '0.75rem', color: '#000', background: C, border: `1px solid ${C}40` }}
                    onMouseEnter={e => { e.currentTarget.style.boxShadow = `0 0 22px ${C}50`; e.currentTarget.style.transform = 'translateY(-2px)' }}
                ...
                >
                  <div style={{ width: '80px', height: '80px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${C}40`, boxShadow: `0 0 30px ${C}15`, background: `${C}08` }}>
          ...
          {/* HOLOGRAM CANVAS — PresentationControls handles its own interaction; outer wrapper is passive */}
          <motion.div
            className="w-full lg:w-[46%] relative mt-16 lg:mt-0"
            style={{ height: 'clamp(360px, 48vw, 520px)' }}
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
          ...
          >
            <Suspense fallback={
              ...
              >
                ...
                <directionalLight position={[-10, -10, -10]} intensity={0.8} color={C} />
                <HologramContactNode />
                <Preload all />
              </Canvas>
            </Suspense>
