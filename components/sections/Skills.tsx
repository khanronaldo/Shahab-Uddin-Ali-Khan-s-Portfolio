...
// Wrapped in React.memo to completely isolate animations and prevent micro-stutters when siblings are hovered
const SkillBubble = React.memo(function SkillBubble({ skill, index, activeIndex, onHover, isMobile }: any) {
    ...
    >
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex justify-center items-center pointer-events-none">
        <motion.div
          ...
          style={{
            height: '85px',
            willChange: 'width, transform', // Forces GPU hardware acceleration for buttersmooth expansion
            ...
            {isHovered && (
              ...
              >
                ...
                <div className="w-full h-[3px] bg-white/10 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }} animate={{ width: `${skill.pct}%` }} className="h-full" style={{ background: skill.color }} 
                  />
                </div>
              </motion.div>
            )}
...
export default function Skills() {
  ...
  // useCallback prevents re-creating the function on every render, keeping memoized children smooth
  ...
  return (
    <>
      ...
      <section ref={sectionRef} id="skills" className="relative w-full bg-black overflow-hidden py-32 px-4">
        ...
        {/* CENTERED CONTENT WRAPPER - Restored w-full and min-h-[70vh] for layout stability */}
        <div className="relative z-10 max-w-7xl mx-auto flex flex-col items-center w-full min-h-[70vh]">
          ...
          >
            ...
            <h2 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-white uppercase tracking-tighter leading-none" style={{ fontFamily: "'Syne',sans-serif" }}>
              Core <span style={{ background: `linear-gradient(90deg, ${P}, ${M}, ${C}, ${M}, ${P})`, backgroundSize: '200% auto', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', animation: 'tubelight-name 3s linear infinite', paddingBottom: '0.1em' }}>Tech Stack</span>
            </h2>

            <p className="mt-8 max-w-2xl text-sm md:text-base font-mono leading-relaxed" style={{ color: 'rgba(255,255,255,0.6)' }}>
              <span style={{ color: C }}>// </span> Building high-performance interfaces with modern primitives. Focused on smooth motion, scalable architecture, and pixel-perfect design realities.
            </p>
          </motion.div>

          {/* BOTTOM SECTION: SKILL BUBBLES GRID */}
